...
if is_installed('swattention'):
    ...
    from backbone.attention_cuda import AggregatedAttention
else:
    print('swattention package not found, loading PyTorch native version of Aggregated Attention')
    from backbone.attention_native import AggregatedAttention
...
@register_model
def transnext_base(pretrained=True, **kwargs):

    # 假设预训练权重文件名为 'pretrained_weights.pth'
    pretrained_weights_path = 'E:/work/backbone/transnext_classify_base_224_1k.pth'
    # # 加载预训练权重
    ...
    model = TransNeXt(window_size=[3, 3, 3, None],
                      ...
                      **kwargs) # 这里的 **kwargs 应包含你之前定义模型时使用的参数

    # 将预训练权重加载到模型
    # 如果需要，可以在这里添加一个 lambda 函数来过滤掉不匹配的键
    model.load_state_dict(pretrained_weights, strict=False)
    # 现在模型已经加载了预训练权重，可以用于推理或进一步的训练
